...
platform_do_upgrade_openmesh()
{
	...
	*)
	...
	local cfg_content=$(dd if="$img_path" bs=1 skip=$data_offset count=$((0x$cfg_size)) 2>/dev/null)
	...
	dd if="$img_path" bs=1 skip=$(($data_offset + 0x$cfg_size + 0x$kernel_size)) count=$((0x$rootfs_size)) 2>&- | mtd -n -p $kernel_length $restore_backup write - $PART_NAME
	dd if="$img_path" bs=1 skip=$(($data_offset + 0x$cfg_size)) count=$((0x$kernel_size)) 2>&- | mtd -n write - $PART_NAME
