...
static int rtl826x_phy_patch_db_init(struct phy_device *phydev, const char *fw_name)
{
	...
	if (ret) {
		phydev_err(phydev, "Failed to request %s\n", fw_name);
	...
	fw_count = fw->size / sizeof(*patches);
	...
	memcpy(patch_db->conf, patches, flex_array_size(patch_db, conf, fw_count));

	if (priv->patch_rtk_serdes) {
		patch_db->conf[fw_count].patch_op = RTK_PATCH_OP_PSDS0;
		patch_db->conf[fw_count].portmask = 0xff;
		patch_db->conf[fw_count].pagemmd = cpu_to_le16(0x07);
		patch_db->conf[fw_count].addr = cpu_to_le16(0x10);
		patch_db->conf[fw_count].msb = 15;
		patch_db->conf[fw_count].lsb = 0;
		patch_db->conf[fw_count].data = cpu_to_le16(0x80aa);

		patch_db->conf[fw_count + 1].patch_op = RTK_PATCH_OP_PSDS0;
		patch_db->conf[fw_count + 1].portmask = 0xff;
		patch_db->conf[fw_count + 1].pagemmd = cpu_to_le16(0x06);
		patch_db->conf[fw_count + 1].addr = cpu_to_le16(0x12);
		patch_db->conf[fw_count + 1].msb = 15;
		patch_db->conf[fw_count + 1].lsb = 0;
		patch_db->conf[fw_count + 1].data = cpu_to_le16(0x5078);
	}
